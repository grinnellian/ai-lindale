...
**Project agent roster (DX-036):** the framework TPM's `Agent` tool is unrestricted — it can dispatch any agent defined in `.claude/agents/`, not just the framework defaults. This means a generated SME (or any other project-owned agent you add by hand) is dispatchable by the TPM immediately, with no edit to `tpm.md` required. Only a project that has claimed a local override of `tpm.md` (see "Claiming a Local Override" above) and re-narrowed its `Agent` list needs to remember to add new agent names to that override by hand.
