...
# 4. Commit everything
git add .ai-lindale .claude scripts/hooks CLAUDE.md .gitmodules
...
Any of the framework-managed paths (core agents, matching commands, hook scripts) can be claimed as a project-local override by replacing the symlink with a real file. `install.sh` will detect the regular file on subsequent runs and skip it — preserving your customization.
