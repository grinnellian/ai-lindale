...
- The reviewer's bar is "satisfied," never "no blockers found": dev must address every review comment, including nits, before a PR is passed for merge. Before accepting `ready-for-review`, check the commit history against the TDD Red/Green Commit Strategy (dev.md, DX-037) — a single monolithic commit for a multi-phase ticket is itself a blocker, not a nit; send it back for re-commit in reviewable increments. Exception: work produced via the BUG-006 stage-and-return fallback legitimately arrives as one commit (see dev.md's single-commit exception). Verify the fallback actually applied before waiving the gate, and verify it from artifacts rather than from the dev's word — the fallback leaves `.claude/commit-msg.txt` in the worktree (dev.md instructs writing it) and the commit's message is the one that file holds. Do **not** try to tell dev-made from TPM-made commits by git author: subagents and the TPM session run under the same local git identity, exactly as they share one GitHub login for comments (see the Escalation Protocol above). A single commit with no `commit-msg.txt` is a plain monolithic commit and still a blocker
...
- `/pr-refresh` is this command's queue-maintenance companion: run it to reconcile and rebase the open-PR queue (conflict matrix, merge order, stacked-PR rebasing) before or between `/autodev` passes, rather than folding that bookkeeping into this state machine
- Before advancing a ticket past any checkpoint where comments were just read (triage, arch review completion, ready-for-review, escalation resume — examples, not an exhaustive list), fold scope/design decisions from the thread into the issue description per the Issue Description as Authoritative Spec protocol (DX-028, see `tpm.md`) — the next agent picking up the ticket reads the description, not the thread
